...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($accion === "guardar_hero") {
        ...
        header("Location: interview.php?ok=1");
        exit;
    }
    ...
    if ($accion === "guardar_seccion") {
        ...
        header("Location: interview.php?ok=1#secciones");
        exit;
    }
    ...
    if ($accion === "guardar_item") {
        ...
        header("Location: interview.php?ok=1#items");
        exit;
    }
}

$page = $conn->query("SELECT * FROM interview_page LIMIT 1")->fetch_assoc();

$sections = $conn->query("
    SELECT * FROM interview_sections
    WHERE estado != 0
    ORDER BY orden ASC, id ASC
");

$sectionsSelect = $conn->query("
    SELECT id, nombre_seccion FROM interview_sections
    WHERE estado != 0
    ORDER BY orden ASC
");

$items = $conn->query("
    SELECT ii.*, ise.nombre_seccion
    FROM interview_items ii
    INNER JOIN interview_sections ise ON ise.id = ii.section_id
    WHERE ii.estado != 0
    ORDER BY ise.orden ASC, ii.orden ASC
");
        ...
        <form method="POST" class="border rounded p-3 mb-3">
            ...
            <div class="row">
                ...
                <div class="col-md-3 mb-3">
                    ...
                    <input class="form-control" name="nombre_seccion" value="<?= htmlspecialchars($s['nombre_seccion']); ?>">
                </div>

                <div class="col-md-5 mb-3">
                    <label>Título</label>
                    <input class="form-control" name="titulo" value="<?= htmlspecialchars($s['titulo']); ?>">
                </div>

                <div class="col-md-2 mb-3">
                    <label>Orden</label>
                    <input type="number" class="form-control" name="orden" value="<?= (int)$s['orden']; ?>">
                </div>
                ...
                <div class="col-md-12 mb-3">
                    <label>Subtítulo</label>
                    <textarea class="form-control" name="subtitulo"><?= htmlspecialchars($s['subtitulo']); ?></textarea>
                </div>

                <div class="col-md-12 mb-3">
                    <label>Contenido</label>
                    <textarea class="form-control" name="contenido"><?= htmlspecialchars($s['contenido']); ?></textarea>
                </div>
    ...
    <?php while($i = $items->fetch_assoc()): ?>
        ...
        <form method="POST" class="border rounded p-3 mb-3">
            ...
            <div class="row">
                    ...
                    <select name="section_id" class="form-control">
                        <?php while($ss = $sectionsSelect->fetch_assoc()): ?>
                            <option value="<?= $ss['id']; ?>" <?= $ss['id']==$i['section_id']?'selected':''; ?>>
                                <?= htmlspecialchars($ss['nombre_seccion']); ?>
                            </option>
                        <?php endwhile; ?>
                    </select>
                </div>
                ...
                <div class="col-md-5 mb-3">
                    <label>Título</label>
                    <input class="form-control" name="titulo" value="<?= htmlspecialchars($i['titulo']); ?>">
                </div>

                <div class="col-md-2 mb-3">
                    <label>Orden</label>
                    <input type="number" class="form-control" name="orden" value="<?= (int)$i['orden']; ?>">
                </div>
                ...
                <div class="col-md-12 mb-3">
                    ...
                    <textarea class="form-control" name="descripcion"><?= htmlspecialchars($i['descripcion']); ?></textarea>
                </div>
