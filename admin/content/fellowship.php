...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($accion === "guardar_hero") {
        ...
        header("Location: fellowship.php?ok=1");
        exit;
    }
    ...
    if ($accion === "guardar_seccion") {
        ...
        $stmt = $conn->prepare("
            UPDATE fellowship_sections SET
                nombre_seccion = ?,
                titulo = ?,
                subtitulo = ?,
                contenido = ?,
        ...
        header("Location: fellowship.php?ok=1#secciones");
        exit;
    }
    ...
    if ($accion === "guardar_item") {
        ...
        header("Location: fellowship.php?ok=1#items");
        exit;
    }
}

$page = $conn->query("SELECT * FROM fellowship_page LIMIT 1")->fetch_assoc();

$sections = $conn->query("
    SELECT * FROM fellowship_sections
    WHERE estado != 0
    ORDER BY orden ASC, id ASC
");

$sectionsSelect = $conn->query("
    SELECT id, nombre_seccion FROM fellowship_sections
    WHERE estado != 0
    ORDER BY orden ASC
");
