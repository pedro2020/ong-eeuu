...
if ($rol !== 'admin' && (int)$_SESSION['puede_crear'] !== 1) {
    die("Acceso denegado");
}
    ...
    if ($usuario && $password) {
        ...
        $stmt = $conn->prepare("
            INSERT INTO usuarios (
                ...
                puede_crear, puede_editar, puede_eliminar, puede_ver_usuarios, estado
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1)
        ");
...
<form method="POST">
    ...
    <div class="form-check">
        <input type="checkbox" name="puede_crear" class="form-check-input">
        <label class="form-check-label">Crear</label>
    </div>

    <div class="form-check">
        <input type="checkbox" name="puede_editar" class="form-check-input">
        <label class="form-check-label">Editar</label>
    </div>

    <div class="form-check">
        <input type="checkbox" name="puede_eliminar" class="form-check-input">
        <label class="form-check-label">Eliminar</label>
    </div>

    <div class="form-check">
        <input type="checkbox" name="puede_ver_usuarios" class="form-check-input">
